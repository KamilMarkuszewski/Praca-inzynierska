...
public class NetworkTransformReceiver : MonoBehaviour
{
    ...
    void ReceiveTransform(SFSObject data)
    {
        ...
        if (receiveMode)
        {
            ...
            int hp = (int)Convert.ToSingle(data.GetNumber("hp"));
            int max_hp = (int)Convert.ToSingle(data.GetNumber("max_hp"));
            bool died = (bool)Convert.ToBoolean(data.GetBool("died"));
            setPlayerHp(hp, max_hp);
    ...
    void setPlayerHp(int _hp, int _max_hp)
    {
        PlayerData pl = gameObject.GetComponent(typeof(PlayerData)) as PlayerData;
        pl.setPlayerHp(_hp, _max_hp);
    }

    void setPlayerDie(bool died)
    {
        PlayerData pl = gameObject.GetComponent(typeof(PlayerData)) as PlayerData;
        pl.setPlayerDied(died);
    }
